...
async def test_command_login_handler_success(mock_msg):
    ######### Create test user in db #############
    async with get_test_db() as db:
        user: User = await BotService.create_user(
            telegram_user=mock_msg.from_user,
            phone_number=mock_msg.contact.phone_number,
            db=db,
        )
        await db.commit()
    ...
    # run login command handler
    await command_login_handler(mock_msg)

    async with get_test_db() as db:
        user: User = await UserRepository.get(tid=mock_msg.from_user.id, db=db)
    ...
    mock_msg.answer.assert_called_once_with(f"🔐 Code: <code>{code.value}</code>")
...
async def test_command_login_handler_object_already_exists():
    ...
    mock_message.answer.assert_called_once_with("Previous passcode is still valid ☝️")


@pytest.mark.asyncio
@patch("src.bot.service.BotService.get_passcode")
async def test_command_login_handler_unexpected_error(mock_get_passcode):
    mock_get_passcode.side_effect = Exception("Unexpected error")
    mock_message = AsyncMock(spec=Message)
    mock_message.from_user = Mock(spec=TelegramUser)
    mock_message.from_user.id = 12345
    mock_message.answer = AsyncMock()
    ...
    mock_get_passcode.assert_awaited_once_with(telegram_user=mock_message.from_user)
